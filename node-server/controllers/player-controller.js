...
exports.streamTorrent = async (req, res) => {
    ...
    try {
        ...
        console.log('test');
        const engine = await setupEngine(magnet, options).then(async engineInstance => {
            ...
            if (!req.headers.range || !movie.file.length || !file_type)
                throw new Error(`Header could not be created headerRange: ${req.headers.range}, totalBytes: ${movie.file.length}, Type: ${file_type}`);
            console.log('test7');
            const position = req.headers.range.replace(/bytes=/, "").split("-");
            const start = parseInt(position[0], 10);
            const end = position[1] ? parseInt(position[1], 10) : movie.file.length - 1;;
        ...
        .pipe(
            streamz(
                async (entry) => {
                    ...
                    const filesExist = await fs.existsSync(`../movies/${imdb_id}/subs/${lang}`);
                    console.log(`fileExist? => ${filesExist}...`);
                    if (!filesExist) await fs.mkdirSync(`../movies/${imdb_id}/subs/${lang}`, { recursive: true });
                    console.log(`${parsedPath.name}.${parsedPath.ext} Saving as ../movies/${imdb_id}/subs/${lang}/${imdb_id}.srt...`);
                    return parsedPath.ext == ".srt"
                        ? entry
                            .pipe(srt2vtt())
                            .pipe(fs.createWriteStream(`../movies/${imdb_id}/subs/${lang}/${imdb_id}.vtt`))
                            .on("error", (e) => {
                                throw new Error(e.message);
                            })
                        : entry
...
exports.getSubtitles = async (req, res) => {
    ...
    let existingSubs = [];
    const enfilesExist = await fs.existsSync(`../movies/${imdb_id}/subs/en/${imdb_id}.vtt`);
    const frfilesExist = await fs.existsSync(`../movies/${imdb_id}/subs/fr/${imdb_id}.vtt`);
